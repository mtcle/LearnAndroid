 ...
 */
public class DebugUtilLogLocal {
    private static DebugUtilLogLocal instance = null;
     ...
     * @return
     */
    public static DebugUtilLogLocal getInstance(Context mContext, String path) {
        if (instance == null) {
            instance = new DebugUtilLogLocal(mContext, path);
        }
        return instance;
    }

    private DebugUtilLogLocal(Context mContext, String path) {
        appid = android.os.Process.myPid();
        if (TextUtils.isEmpty(path)) {
            dirPath = Environment.getExternalStorageDirectory().getAbsolutePath()
                    + File.separator + "1seeker" + File.separator + mContext.getPackageName();
        } else {
            dirPath = path;
        }
