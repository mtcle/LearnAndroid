            ...
            if (null != res) {
                ...
                if (res.isSuccessful()) {
                    if (helper.getBusinessType() == BusinessType.HttpOrHttps
                        ...
                        return retInfo(info, netCode, HttpInfo.SUCCESS, str);
                    ...
                    } else if (netCode == 401) {//请求需要认证
                        return retInfo(info, netCode, HttpInfo.RequestForbiddened);
                    } else if (netCode == 403) {//请求页面路径错误
                        return retInfo(info, netCode, HttpInfo.RequestForbiddened);
                    } else if (netCode == 404) {//请求页面路径错误
                        return retInfo(info, netCode, HttpInfo.ServerNotFound);
                    } else if (netCode == 405) {//请求类型错误
                        return retInfo(info, netCode, HttpInfo.RequestMethodError);
